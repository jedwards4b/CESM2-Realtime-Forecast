...
def build_base_case(date, baseroot, basemonth,res, compset, overwrite,
    ...
    caseroot = os.path.join(baseroot,"cesm2cam6climoALL.{:02d}".format(basemonth)+".00")
    ...
    with Case(caseroot, read_only=False) as case:
        if not os.path.isdir(caseroot):
            ...
            # pelayout for cesm2cam6climoALL case
...
def _main_func(description):
    ...
    sdrestdir = os.path.join(os.getenv("SCRATCH"),"cesm2cam6climoALL","Ocean","rest","{}".format(date))

    user_mods_dir = os.path.join(s2sfcstroot,"user_mods","cesm2cam6climoALL")
