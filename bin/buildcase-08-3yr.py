...
def build_base_case(date, baseroot, basecasename, basemonth,res, ensemble_start, compset, overwrite,
    ...
    with Case(caseroot, read_only=False) as case:
        if not os.path.isdir(caseroot):
            ...
            case.set_value("STOP_N", 17)
            case.set_value("REST_OPTION","nmonths")
            case.set_value("REST_N", 17)
...
def clone_base_case(date, caseroot, ensemble_start, ensemble_end, sdrestdir, user_mods_dir, overwrite):
    ...
    for i in range(ensemble_start+1, ensemble_end+1):
        ...
        sdrestdir = os.path.join("/glade","scratch","nanr","SMYLE-DP","archive","b.e21.BSMYLE.f09_g17."+date[0:7]+".{0:03d}".format(i),"rest","{0:04d}-08-01-00000".format(baseyear+2))
...
def _main_func(description):
    ...
    overwrite = True
    user = os.getenv("USER")
    sdrestdir = os.path.join("/glade","scratch","{}".format(user),"SMYLE","archive","b.e21.BSMYLE.f09_g17."+date[0:7]+".{0:03d}".format(ensemble_start),"rest","{0:04d}-08-01-00000".format(baseyear+2))
