...
contains
  ...
  subroutine extfrc_inti( extfrc_specifier, extfrc_type_in, extfrc_cycle_yr, extfrc_fixed_ymd, extfrc_fixed_tod)
    ...
    character(len=256) :: frc_fnames(max_num_emis_files)
    real(r8)           :: frc_scalefactor(max_num_emis_files)
    character(len=16)  :: frc_species(max_num_emis_files)
    integer            :: frc_indexes(max_num_emis_files)
    ...
    count_emis: do n=1,max_num_emis_files
       ...
       if (masterproc) then
          write(iulog,'(a,i3,a)') 'extfrc_inti n: ',n,' extfrc_specifier: '//trim(extfrc_specifier(n))
       endif
