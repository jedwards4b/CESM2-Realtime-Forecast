...
contains
  ...
  subroutine srf_emissions_inti( srf_emis_specifier, emis_type_in, emis_cycle_yr, emis_fixed_ymd, emis_fixed_tod )
    ...
    integer, parameter :: max_num_emis_files = 100

    character(len=16)  :: emis_species(max_num_emis_files)
    character(len=256) :: emis_filenam(max_num_emis_files)
    integer  :: emis_indexes(max_num_emis_files)
    integer  :: indx(max_num_emis_files)
    real(r8) :: emis_scalefactor(max_num_emis_files)
    ...
    count_emis: do n=1,max_num_emis_files

       if (masterproc) then
          write(iulog,'(a,i3,a)') 'srf_emissions_inti n: ',n,' srf_emis_specifier: '//trim(srf_emis_specifier(n))
       endif
