...
!                end do
    ...
    if (any(fail_cnt > 0)) then
       do i=1,avec_len
          if(fail_cnt(i) > 0) then
             write(iulog,'('' imp_sol: time step '',1p,g15.7,'' failed to converge @ (i,lchnk,vctrpos,nstep, fail_cnt) = '',5i8)') &
                  dt(i),i,lchnk,ofl+i-1,nstep, fail_cnt(i)
          endif
       enddo
    endif
