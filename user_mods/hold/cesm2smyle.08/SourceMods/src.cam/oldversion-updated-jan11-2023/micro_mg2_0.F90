...
!++ MCB
  use MCB_mask, only: get_MCB_mask
...
!++ MCB 
  real(r8) :: MCB_seeding_mask(pcols)
...
!++ MCB
  call get_MCB_mask(lchnk, ncol, MCB_seeding_mask)
!-- MCB
...
!++ MCB
  ! change nctend to make ncic = 375 #/cm3
  do i=1,mgncol
     if(MCB_seeding_mask(i)>=0.95_r8) then
      do k=1,nlev
         if(p(i,k)>=85000._r8) then
            nctend(i,k) = (3.75e8_r8/rho(i,k)*lcldm(i,k)-ncn(i,k))/deltatin
         end if
      end do
     end if
  end do
