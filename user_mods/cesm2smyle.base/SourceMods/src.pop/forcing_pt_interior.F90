...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      pt_interior_shr_stream_year_first, & ! first year in stream to use
      pt_interior_shr_stream_year_last,  & ! last year in stream to use
      pt_interior_shr_stream_year_align    ! align pt_interior_shr_stream_year_first with this model year

   character(char_len) :: &
      pt_interior_shr_stream_file          ! file containing domain and input data

   integer (int_kind) :: &
      tavg_TEMP_RESTORE,                 & ! tavg id for PT_INTERIOR_DATA
      tavg_TEMPTEND_RESTORE                ! tavg id for PT_INTERIOR_DATA tendency term

   type(strdata_input_type) :: &
      pt_inputlist                         ! pop stream datatype

   integer (int_kind) :: &
      pt_interior_shr_strdata_advance_timer  ! timer
...
!-----------------------------------------------------------------------
   ...
   case ('shr_stream')
      ...
      if (my_task == master_task) then
         write(stdout,blank_fmt)
         write(stdout,'(a)') ' Interior PT shr_stream options: '
         write(stdout,'(a15,2x,i10)') '   year_first = ', pt_interior_shr_stream_year_first
         write(stdout,'(a15,2x,i10)') '   year_last = ', pt_interior_shr_stream_year_last
         write(stdout,'(a15,2x,i10)') '   year_align = ', pt_interior_shr_stream_year_align
         write(stdout,'(2a)') '   file = ', pt_interior_shr_stream_file
      endif
...
!     pt_data_in = construct_io_field('PT_RESTORE_RTAU', dim1=i_dim, dim2=j_dim, &
      pt_data_in = construct_io_field('PT_RESTORE_MASK', dim1=i_dim, dim2=j_dim, &
                             field_loc = field_loc_center,             &
                             field_type = field_type_scalar,           &
                             d2d_array = PT_RESTORE_RTAU)
...
!     PT_RESTORE_RTAU = PT_RESTORE_RTAU/seconds_in_day ! convert days to secs
      PT_RESTORE_RTAU = PT_RESTORE_RTAU*pt_interior_restore_rtau
      call destroy_io_field(pt_data_in)
      call data_set (pt_int_data_file, 'close')
      call destroy_file(pt_int_data_file)
...
!-----------------------------------------------------------------------
   ...
   call define_tavg_field(tavg_TEMP_RESTORE,'TEMP_RESTORE',3,              &
                          long_name='T values of interior restoring data', &
                          units='degC', grid_loc='3111',         &
                          coordinates='TLONG TLAT z_t time')
   call define_tavg_field(tavg_TEMPTEND_RESTORE,'TEMP_RESTORE_TEND',3,              &
                          long_name='Interior TEMP restoring tendency', &
                          units='degC/s', grid_loc='3111',         &
                          coordinates='TLONG TLAT z_t time')
!-----------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------
   ...
   case ('shr_stream')
      ...
      do iblock = 1, nblocks_clinic
         ...
         !do k=1,km
            ...
            do i=this_block%ib,this_block%ie
               n = n + 1
               PT_INTERIOR_DATA(i,j,1,iblock,1) = &
      ...
      if (errorCode /= POP_Success) then
         call exit_POP(sigAbort, 'get_pt_interior_data' /&
...
!-----------------------------------------------------------------------
      ...
      call accumulate_tavg_field(DPT_INTERIOR, tavg_TEMPTEND_RESTORE, bid, k)
