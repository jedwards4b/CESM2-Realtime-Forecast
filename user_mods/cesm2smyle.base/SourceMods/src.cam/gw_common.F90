...
! Physics buffer indices 
   integer     ::  gwave_dp_idx      = 0
   integer     ::  gwave_ux_idx      = 0
   integer     ::  gwave_kwv_idx     = 0
#endif
...
!==========================================================================
#ifdef CHEMWAVE
subroutine gw_common_register ( pcols_in, pver_in )
  use physics_buffer, only : pbuf_add_field, dtype_r8
  implicit none

  integer, intent(in) :: pcols_in, pver_in

  call pbuf_add_field( 'GW_WAVE_DP',   'global', dtype_r8, (/pcols_in, pver_in+1/) , gwave_dp_idx)
  call pbuf_add_field( 'GW_WAVE_UX',   'global', dtype_r8, (/pcols_in, pver_in  /) , gwave_ux_idx)
  call pbuf_add_field( 'GW_WAVE_KWV',  'global', dtype_r8, (/pcols_in /) ,           gwave_kwv_idx)

end subroutine gw_common_register
#endif
!==========================================================================
...
#ifdef CHEMWAVE
         ,lget_waveq_in, wave_vertdisp, wave_vwavenum &
         ,gwut_00,gwut_01 &
#endif
     )
...
#ifdef CHEMWAVE
!++ chem
  logical, intent(in), optional             :: lget_waveq_in
  real(r8),intent(out),allocatable,optional :: wave_vertdisp(:,:,:)
  real(r8),intent(out),allocatable,optional :: wave_vwavenum(:,:,:)
  real(r8),intent(out),allocatable,optional :: gwut_00(:,:,:)
  real(r8),intent(out),allocatable,optional :: gwut_01(:,:,:)
#endif
  ...
  ! Saturation stress.
  real(r8) :: tausat(ncol)
  ! (ub-c) and (ub-c)**2 and abs(ubmc)+0.1
...
!--
  ...
  ! Gravity wave wind tendency for each wave.
  real(r8) :: gwut_surreal( ncol, pver, -band%ngwv:band%ngwv )

  logical :: lapply_effgw , lget_waveq, do_vertical_diffusion
...
#ifdef CHEMWAVE
  if (present(lget_waveq_in)) then
     lget_waveq = lget_waveq_in
     if (lget_waveq_in) then
        allocate( wave_vertdisp(ncol,-band%ngwv:band%ngwv,pver+1) )
        allocate( wave_vwavenum(ncol,-band%ngwv:band%ngwv,pver+1) )
        wave_vertdisp(:,:,:) =0._r8
        wave_vwavenum(:,:,:) =0._r8
        allocate( gwut_00(ncol, pver, -band%ngwv:band%ngwv ) )
        allocate( gwut_01(ncol, pver, -band%ngwv:band%ngwv ) )
        gwut_00(:,:,:) =0._r8
        gwut_01(:,:,:) =0._r8
     endif
  else
...
#ifdef CHEMWAVE
        if (lget_waveq) then
           wave_vwavenum(:,l,k) = ni(:,k) / ubmca  ! max( abs(ubmc) , 0.1_r8 )
           if (present( kwvrdg )) then
              where (src_level >= k)
                wave_vertdisp(:,l,k) = sqrt( tau(:,l,k) / ( rhoi(:,k) * ni(:,k) * ubmca * kwvrdg ) )
              end where
           else
              where (src_level >= k)
                 wave_vertdisp(:,l,k) = sqrt( tau(:,l,k) / ( rhoi(:,k) * ni(:,k) * ubmca * band%kwv ) )
              end where
           end if
        end if
#endif
...
!!#if 0
  ...
  do k = ktop, kbot_tend
     do l = -band%ngwv, band%ngwv    ! loop over wave
         gwut_00(:,k,l) = gravit * (tau(:,l,k+1)-tau(:,l,k)) * p%rdel(:,k)
     end do
  end do
  end if
#endif
...
!!#if 0
  ...
  do k = ktop, kbot_tend
     do l = -band%ngwv, band%ngwv      ! loop over wave
         !!gwut_surreal(:,k,L) = gwut(:,k,L)
         gwut_01(:,k,L) = gravit * (tau(:,L,k+1)-tau(:,L,k)) * p%rdel(:,k)
     end do
  end do
  end if
#endif
