...
character(len=*), parameter :: mask_file = '/glade/work/jesswan/scripps/SMYLE-MCB/mask_CESM/sesp_mask_CESM2_0.9x1.25_v6.nc'
...
subroutine get_MCB_mask(lchnk, ncol, mask)
       ...
       if(yr==2015 .AND. mon>=6) then
          do i=1,ncol
             mask(i) = mask2D(lons(i), lats(i), mon)
          end do
       else if (yr==2016 .AND. mon<=2) then
          do i=1,ncol
             mask(i) = mask2D(lons(i), lats(i), mon)
          end do
       else 
