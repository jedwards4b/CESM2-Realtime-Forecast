...
def get_rvals(date, ensemble_start,ensemble_end, model):
    ...
    print("1st = {}".format(date[0:4]))
    print("2nd = {}".format(date[5:7]))
    print("3rd = {}".format(date[8:10]))

    rvals = random.sample(range(1001),k=ensemble_end//2)
    print("Rvals are {}".format(rvals))
    sys.exit()
    rvals_file = os.path.join("/glade/p/cesm/espwg/CESM2-SMYLE/","cases","testcamic_"+date+".{}-{}.txt".format(ensemble_start,ensemble_end))
